...
/**
 * Select previous row with difference in table.
 *
 * @author snake
 *
 */
public final class PrevDifferenceAction extends AbstractAction implements Action, ChangeListener {
	 ...
	 *            main frame
	 */
	public PrevDifferenceAction(final MainFrame frame) {
		this.frame = frame;
		this.leftTable = null;
	...
	public void actionPerformed(final ActionEvent e) {
		if (leftTable != null && rightTable != null) {
			...
			if (nRows > 0) {
				int index = DifferenceUtil.getMaxSelectedRow(leftTable.getSelectedRow(), rightTable.getSelectedRow());
				int nextIndex = DifferenceUtil
					.getMaxSelectedRow(leftModel.getPrevDifference(index), rightModel.getPrevDifference(index));
